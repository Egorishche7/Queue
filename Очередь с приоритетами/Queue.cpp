...
{
	if (HEAD == NULL)
		throw exception("\nУдаление элемента невозможно, очередь пуста!\n");
...
string Queue::getHeadInfo()
{
	...
	return "Значение элемента: " + to_string(HEAD->getValue()) + "    Приоритет: " + to_string(HEAD->getPriority());
}
...
ostream& operator << (ostream& out, const Queue& queue)
{
	...
	while (cursor != NULL)
	{
		out << "Значение: " << cursor->getValue() << "    Приоритет: " << cursor->getPriority() << "\n";
		cursor = cursor->getNextItem();
	}
	return out;
}
